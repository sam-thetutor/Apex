import { pineconeKnowledgeService } from '../vector/pinecone-knowledge'
...
export async function addInitialBaseKnowledgeToPinecone(): Promise<void> {
  console.log('Adding initial Base knowledge to Pinecone...')
  ...
  try {
    ...
    for (const knowledge of initialBaseKnowledge) {
      await pineconeKnowledgeService.addDocument(knowledge)
      console.log(`✅ Added: ${knowledge.title}`)
    }
    
    console.log('🎉 Initial Base knowledge added to Pinecone successfully!')
...
if (require.main === module) {
  addInitialBaseKnowledgeToPinecone()
    .then(() => {
